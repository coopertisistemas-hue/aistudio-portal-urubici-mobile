      ...
      <div className="flex gap-4">
        <div className="w-20 h-20 rounded-xl overflow-hidden bg-white/20 flex-shrink-0">
          <img
            ...
            src={`https://readdy.ai/api/search-image?query=$%7BencodeURIComponent%28%60$%7Bestablishment.name%7D%20artisan%20craft%20shop%20in%20Urubici%20Santa%20Catarina%20Brazil%20with%20handmade%20products%20local%20souvenirs%20and%20authentic%20craftsmanship%60%29%7D&width=200&height=200&seq=${establishment.id}&orientation=squarish`}
          />
        </div>
        ...
        <div className="flex-1">
          <div className="flex items-start justify-between mb-2">
            <div className="flex-1">
              ...
              {establishment.address && (
                <p className="w-full break-words overflow-hidden line-clamp-2 text-white/70 text-xs mb-2">
                  <i className="ri-map-pin-line text-amber-400 mr-1" />
                  {establishment.address}
            ...
            <div className="flex flex-col gap-1.5 items-end ml-2">
              {/* Chip de tipo */}
              <div className="bg-amber-500/20 text-amber-200 border-amber-400/30 px-2 py-1 rounded-full text-xs font-medium whitespace-nowrap">
                {groupName.slice(0, -1)}
              </div>
          ...
          {establishment.description && (
            <p className="w-full break-words overflow-hidden line-clamp-3 text-white/80 text-xs leading-relaxed mb-3">
              {establishment.description}
            </p>
          )}
          ...
          {establishment.services_offered && establishment.services_offered.length > 0 && (
            <div className="flex flex-wrap gap-1 mb-3">
              {establishment.services_offered.slice(0, 3).map((service, idx) => (
                <div
                  key={idx}
                  className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium border bg-amber-500/20 text-amber-200 border-amber-400/30"
                >
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            {/* Botões dinâmicos específicos para artesanato */}
            {establishment.instagram_url && (
              <a
                href={establishment.instagram_url}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center justify-center gap-2 px-3 py-2.5 bg-gradient-to-br from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 hover:scale-105 rounded-lg text-white text-xs font-medium transition-all duration-200 whitespace-nowrap min-h-[44px] cursor-pointer"
                title="Ver Instagram"
              >
            ...
            {establishment.facebook_url && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center justify-center gap-2 px-3 py-2.5 bg-blue-600 hover:bg-blue-700 hover:scale-105 rounded-lg text-white text-xs font-medium transition-all duration-200 whitespace-nowrap min-h-[44px] cursor-pointer"
                title="Ver Facebook"
              >
            ...
            {establishment.catalog_url && (
              <a
                href={establishment.catalog_url}
                target="_blank"
                rel="noopener noreferrer"
                className="col-span-2 inline-flex items-center justify-center gap-2 px-3 py-2.5 bg-teal-600 hover:bg-teal-700 hover:scale-105 rounded-lg text-white text-xs font-medium transition-all duration-200 whitespace-nowrap min-h-[44px] cursor-pointer"
                title="Ver catálogo"
              >
            ...
            {establishment.website_url && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="col-span-2 inline-flex items-center justify-center gap-2 px-3 py-2.5 bg-indigo-600 hover:bg-indigo-700 hover:scale-105 rounded-lg text-white text-xs font-medium transition-all duration-200 whitespace-nowrap min-h-[44px] cursor-pointer"
                title="Visitar site"
              >
