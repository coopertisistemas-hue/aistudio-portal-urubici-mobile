      ...
      <div className="flex gap-4">
        <div className="w-20 h-20 rounded-xl overflow-hidden bg-white/20 flex-shrink-0">
          <img
            ...
            src={`https://readdy.ai/api/search-image?query=$%7BencodeURIComponent%28%60$%7Bestablishment.name%7D%20supermarket%20grocery%20store%20in%20Urubici%20Santa%20Catarina%20Brazil%20with%20fresh%20products%20and%20quality%20service%60%29%7D&width=200&height=200&seq=${establishment.id}&orientation=squarish`}
          />
        </div>
        ...
        <div className="flex-1">
          <div className="flex items-start justify-between mb-2">
            <div className="flex-1">
              ...
              {establishment.address && (
                <p className="text-white/70 text-xs mb-2 w-full break-words overflow-hidden line-clamp-2">
                  <i className="ri-map-pin-line text-emerald-400 mr-1" />
                  {establishment.address}
            ...
            <div className="flex flex-col gap-1.5 items-end ml-2">
              {/* Chip de tipo */}
              <div className="bg-emerald-500/20 text-emerald-200 border-emerald-400/30 px-2 py-1 rounded-full text-xs font-medium whitespace-nowrap">
                {groupName.slice(0, -1)}
              </div>
          ...
          {establishment.services_offered && establishment.services_offered.length > 0 && (
            <div className="flex flex-wrap gap-1 mb-3">
              {establishment.services_offered.slice(0, 3).map((service, idx) => (
                <div
                  key={idx}
                  className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium border bg-emerald-500/20 text-emerald-200 border-emerald-400/30"
                >
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <button
              disabled
              className="col-span-2 inline-flex items-center justify-center gap-2 px-3 py-2.5 bg-emerald-600/50 cursor-not-allowed rounded-lg text-white text-xs font-medium opacity-50 whitespace-nowrap min-h-[44px]"
              title="Em breve"
            >
