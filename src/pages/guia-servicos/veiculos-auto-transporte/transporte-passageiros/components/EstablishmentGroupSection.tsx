...
import type { Establishment } from '../utils/classification';
        ...
        <div id={`group-${groupName}`}>
            <div className="mb-4">
                <h4 className="text-white font-bold text-lg flex items-center gap-2">
                    <i className="ri-steering-line text-amber-400 flex-shrink-0" />
                    <span className="max-w-[220px] truncate whitespace-nowrap overflow-hidden text-ellipsis">
                        {groupName}
                    </span>
                </h4>
                <p className="text-white/60 text-xs mt-1">
                    {establishments.length} {establishments.length === 1 ? 'opção' : 'opções'}
                </p>
            </div>
