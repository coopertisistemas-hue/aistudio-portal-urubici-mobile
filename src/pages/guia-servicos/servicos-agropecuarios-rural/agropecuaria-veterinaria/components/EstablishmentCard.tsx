...
function getImageUrl(establishment: Establishment): string {
  const query = encodeURIComponent(
    `${establishment.name} agricultural veterinary store in Urubici Santa Catarina Brazil with quality products and professional service`
  );
  return `https://readdy.ai/api/search-image?query=$%7Bquery%7D&width=200&height=200&seq=${establishment.id}&orientation=squarish`;
}
...
export default function EstablishmentCard({ establishment, groupName }: EstablishmentCardProps) {
  ...
  const imageUrl = getImageUrl(establishment);
